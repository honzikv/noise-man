...
class TrackConfig(BaseModel):
    type: str  # white, pink, brown, rain, ocean, space, forest, blue, violet
...
def apply_lowpass(
    data: np.ndarray, cutoff: float, fs: int = 44100, order: int = 4
) -> np.ndarray:
    nyq = 0.5 * fs
    normal_cutoff = cutoff / nyq
    b, a = signal.butter(order, normal_cutoff, btype="low", analog=False)
    return cast(np.ndarray, signal.filtfilt(b, a, data))
...
def generate_blue(num_samples: int) -> np.ndarray:
    # Blue noise: +3dB/octave. We can approximate by differentiating pink noise,
    # or by applying a high-pass to white noise.
    white = np.random.uniform(-1.0, 1.0, num_samples)
    return apply_highpass(white, 2000)


def generate_violet(num_samples: int) -> np.ndarray:
    # Violet noise: +6dB/octave. It's the derivative of white noise.
    white = np.random.uniform(-1.0, 1.0, num_samples + 1)
    violet = np.diff(white)
    max_val = np.max(np.abs(violet))
    if max_val > 0:
        violet = violet / max_val
    return violet * 0.5
...
def export_mix(req: ExportRequest) -> Response:
    ...
    for track in req.tracks:
        ...
        elif track.type == "blue":
            data = generate_blue(num_samples)
        elif track.type == "violet":
            data = generate_violet(num_samples)
        else:
    ...
    # Seamless loop without volume dip: overlap-add the ends
    # Instead of fading to zero, we take a bit from the end and add to the beginning,
    # but we generated exactly `num_samples`. A simple trick for noise is to crossfade
    # the last `fade` samples with the first `fade` samples using equal-power crossfade.
    fade_samples = min(sample_rate * 2, num_samples // 2)
    if fade_samples > 0:
        t = np.linspace(0, np.pi / 2, fade_samples)
        fade_in = np.sin(t)
        fade_out = np.cos(t)

        # We blend the start and end
        start_segment = mix[:fade_samples] * fade_in
        end_segment = mix[-fade_samples:] * fade_out

        # Mix them together
        blended = start_segment + end_segment
        mix[:fade_samples] = blended
        mix[-fade_samples:] = blended
    ...
    return Response(
        ...
        headers={
            "Content-Disposition": f"attachment; filename=loopable-mix-{req.duration_seconds}s.wav"
        },
    )
